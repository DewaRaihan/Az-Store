...
class MonthlyProfitMarginStats extends BaseDateRange implements CardStatsInterface
{
    ...
    {
        return 'monthlyProfitMargin_stats:' . ($filter ?? '');
    }

    public function handle(string|null $filter = null): mixed
    {
        $key = $this->cacheKey($filter);
        return Cache::remember($key, now()->addSeconds(10), fn() => $this->calculate($filter));
    }
}